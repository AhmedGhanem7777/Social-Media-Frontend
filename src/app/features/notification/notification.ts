...
import { NotifFilter } from '../../core/models/tab';

@Component({
  selector: 'app-notification',
  standalone: true,
  imports: [CommonModule, PostDatePipe],
...
export class Notifications implements OnInit {
  ...
  // Load notifications with pagination and filtering
    ...
    this.notificationService.GetNotifications({
      ...
      next: (res) => {
        if (res.isSuccess) {
          ...
          if (append) {
            this.items.update(prev => [...prev, ...newData]);
          } else {
            this.items.set(newData);
          }
          this.hasMore.set(newData.length === this.pageSize);
        }
  ...
  markRead(notif: Notification): void {
    ...
    this.notificationService.ReadNotification(notif.id).subscribe((res) => {
      if (res.isSuccess) {
  ...
  getFilterType(filter: NotifFilter): number {
    switch (filter) {
      case 'likes': return 5;
      case 'comments': return 6;
      case 'follows': return 4;
      default: return 0;
    }
  }

  // Load more notifications when user clicks "Load More" button
  loadMore(): void {
  ...
  onFilterChange(filter: any): void {
    this.activeFilter.set(filter as NotifFilter);
    this.pageIndex = 1;
    this.items.set([]);
    this.loadNotifications();
  }
  ...
  // Icon helpers
  notifBadgeClass(type: string | number): string {
  ...
  notifIcon(type: string | number): string {
